...
import { Pressable, View, Text, TextInput, TouchableOpacity, StyleSheet, Modal, FlatList, ActivityIndicator, Alert, Button, Image, SafeAreaView  } from 'react-native';
...
export default function ImprestSettlementStore() {
    ...
    const handleDateChange = (type, date) => {
        if (date) {
            ...
            // Log the formatted date here directly from the `date` parameter
            //console.log("Exp Date is:", formatDate(date)); // Log the formatted date immediately
        }
    };
    ...
    const pickDocument = async () => {
        try {
            ...
            if (result && result.assets && result.assets[0] && !result.canceled) {
                ...
                const loggedInEmpStore = await AsyncStorage.getItem('officeCode') || 'DEFAULT_STORE'; // Fallback if officeCode is not available
                const randomNo = Math.floor(Math.random() * 9000) + 1000; // Generate a random number
                ...
                if (file.size > 5 * 1024 * 1024) {
                  setAlert({
                      ...
                      message: "Please select an file smaller than 5 MB.",
                      type: "warning",
                    });
                    setPdfUri(null);
                } else {
                ...
                setAlert({
                  ...
                  message: "No document file is selected to upload.",
                  type: "warning",
                });
            }
        } catch (error) {
          ...
          setAlert({
            visible: true,
            title: "error",
            message: "Error occurred while picking document" || error.message,
            type: "error",
          });
        }
    };    
    ...
    <SafeAreaView style={{ flex: 1 }}>
        <FlatList
                ...
                <View style={styles.container}>
                    <View style={styles.section}>
                        <View style={styles.tabInputContainer}>
                            ...
                            <TextInput
                                style={[styles.input, { width: '86%', color: '#333', backgroundColor: '#dedfe0', marginLeft: -27, paddingLeft: 33 }]}
                                placeholder="--Select--"
                                value={expHead}
                        ...
                        >
                          {loading ? (
                            ...
                            <>
                              ...
                              <Text style={{ paddingHorizontal: 20, color: '#fff', fontSize: 16 }}>Submit</Text>
                            </>
                          )}
          ...
          />        
            ...
            showsVerticalScrollIndicator={true}
            //contentContainerStyle={{ paddingBottom: 100 }} // optional: adds space at bottom for button visibility
        /> 
    </SafeAreaView>
)};
