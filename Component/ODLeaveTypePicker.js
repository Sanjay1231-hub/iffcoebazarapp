...
const ODLeaveTypePicker = ({ selectedValue = '', onValueChange = () => {} }) => {

  const [modalVisible, setModalVisible] = useState(false);
  const [alert, setAlert] = useState({ visible: false, title: "", message: "", type: "" });
  const options = [
    { label: '--Select--', value: '' },
    { label: 'On Duty', value: 'DU' },
    { label: 'Forget In-Punch', value: 'IN' },
    { label: 'Forget Out-Punch', value: 'ON' },
  ];
  ...
  const handleSelect = (value) => { 
    ...
    if (value === '') {
      setAlert({ visible: true, title: "Validation Error", message: "Please select a valid OD / Time loss reason option.", type: "warning" });
      return;
    }
...
export default ODLeaveTypePicker;
